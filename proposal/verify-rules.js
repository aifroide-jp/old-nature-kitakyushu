...
//
//   node proposal/verify-rules.js
//
// proposal/mockup-bad/ はわざと全ルールに違反させたモック。ここに lint をかけて、
...
const BAD = path.join(ROOT, 'mockup-bad');
...
function main() {
  ...
  const r = spawnSync('node', [path.join(ROOT, 'lint', 'lint.js'), BAD, '--json'], { encoding: 'utf8' });
  let issues;
  try {
  ...
  if (missing.length) {
    ...
    console.log('  proposal/mockup-bad/ に違反例を足すか、ルールの実装を見直してください。');
  }
  process.exit(1);
}
