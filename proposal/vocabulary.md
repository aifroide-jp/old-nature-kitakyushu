...
- **無限スクロールのカルーセルで、複製カードをどう作るかが未定義。** 実測: トップの
  「活動拠点」は CSS アニメーション（`animation: facilities-auto-scroll 50s linear infinite`）で
  流しており、途切れなく繋ぐには DOM に同じカードの複製が必要（JS では複製していない）。
  モックにはその複製が `aria-hidden="true"` で書かれているが、変換後は `data-loop` が
  実データ N 件を1周ぶん出すだけなので、**モックでは動くが生成物では途切れる**。
  `data-loop` に「2周出力する」宣言を足すのが素直だが未実装。
  **いちばん見つけにくい壊れ方（モックを見ても気づけない）なので、実物変換に着手する時に必ず戻ること。**
