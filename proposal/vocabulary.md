...
| L23 | `data-acf-type="wysiwyg"` の中に `data-acf` / `data-acf-url` を書かない（2.6節）。※22番はナビ形状の固定ルールだったが撤回・削除済み。欠番のまま再利用しない | error |
...
- **実行方法**: `node proposal/a11y/check.js [mockupDir]`。
  `proposal/mockup-real/` 配下の全 `*.html` に pa11y を `--standard WCAG2AA --runner axe`
...
- ~~内部参照が絶対パス（`/about/x.html`）と相対パス（`../about/x.html`）のどちらの記法か未定義~~
  ...
  v0.1 の実例（当時の `proposal/mockup/`。役割を終えて削除済み）がルート絶対パスを使っていたために発覚した。
