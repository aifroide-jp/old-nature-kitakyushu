...
//
//   node proposal/snapshot.js            … 期待値と比較。1バイトでも違えば非ゼロ終了
//   node proposal/snapshot.js --update   … 期待値を現在の出力で更新（意図した変更のときだけ）
//
// なぜ要るか:
...
//   ICHIKI_SCAN="node …/scan.js"  ICHIKI_BUILD="node …/build.js"  node proposal/snapshot.js
...
const MOCKUP = path.join(ROOT, 'mockup-real');
const EXPECTED = path.join(ROOT, 'snapshot', 'expected.json');
...
function main() {
  ...
  if (!fs.existsSync(EXPECTED)) {
    console.error('期待値がありません。`node proposal/snapshot.js --update` で作成してください');
    process.exit(2);
  }
  ...
  console.log('意図した変更なら `node proposal/snapshot.js --update` で凍結し直してください。');
